...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        # --------------------------------------------------------------
        left = ''
        original_len = len(self.contents)
        ...
        if len(self.contents) + len(additional_contents) <= self.volume:
            ...
            for k in range(len(self.contents) - original_len, len(additional_contents)):
    ...
    def double(self):
        ...
        if len(self.contents) * 2 <= self.volume:
            ...
            for k in range(self.volume - len(self.original_contents), len(self.original_contents)):
                left = left + self.original_contents[k]
            return left
    ...
    def shrink(self, new_volume):
        ...
        if self.volume < len(self.contents):
            for k in range(self.volume):
                keep += self.contents[k]
            for k in range(self.volume, len(self.contents)):
                left += self.contents[k]
            self.contents = keep
            return left
        else:
            return ''
    ...
    def double_then_shrink(self, new_volume):
        ...
        left = self.double()        # do it here rather than directly after return so that it can be used when it shrink
        return len(self.shrink(new_volume)) + len(left)
